...
public class LoginAndRegisterController {
    ...
    public Result login(@RequestBody @Valid LoginAndRegisterDto loginAndRegisterDto, BindingResult bindingResult,
        ...
        try {
            resultMap = userService.login(request, loginAndRegisterDto.getEmail(), loginAndRegisterDto.getPassword());
        } catch (Exception e) {
            log.error("登入阶段出现异常，异常信息为：{}", e.getMessage());
            return Result.failForSystem("系统出错，登入异常，请稍后再试");
        }
        return parseMapToResult(resultMap, session);
    }
    ...
    public Result register(@RequestBody @Valid LoginAndRegisterDto loginAndRegisterDto, BindingResult bindingResult) {
        ...
        // 检测验证码是否符合
        if (!userService.checkVerifyCode(user.getEmail(), loginAndRegisterDto.getVerifyCode())) {
     ...
     * @return
     */
    public static Result parseMapToResult(Map<String, String> map, HttpSession session) {
