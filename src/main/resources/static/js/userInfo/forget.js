...
sendBtn.addEventListener("click", function () {
    ...
    $.ajax({
        url: 'https://39b456804w.oicp.vip/common/userInfo/forget/verify/' + emailValue,
        type: 'get',
        success: data => {
...
submitBtn.addEventListener('click', () => {
    ...
    $.ajax({
        ...
        url: 'https://39b456804w.oicp.vip/common/userInfo/forget/resetPassword',
        type: 'post',
        data: JSON.stringify(jsonData),
        success: function(data) {
            submitBtn.disable = false;
            if (data['status'].startsWith("0")) {
                cocoMessage.info("修改成功，即将跳转登入页面");
                var time = 3;
                let timer = setInterval(function () {
                    // 判断剩余秒数
                    if (time === 0) {
                        clearInterval(timer);
                        window.location.href = "https://39b456804w.oicp.vip/loginAndRegister";
                    }
                    time --;
                }, 1000);
            }
